...
const getProfile = async function (req, res) {
  try {
    ...
    res.status(200).send({ status: true, message: "Sucess", data: userDoc });
  } catch (err) {
  ...
  try {
    ...
    if (address?.shipping) {
      ...
      if (shipping.hasOwnProperty("pincode")) {
        ...
        if (!isValidPincode(shipping.pincode) || typeof (shipping.pincode) !== "number") {
          return res.status(400).send({
            status: false,
    ...
    if (address?.billing) {
      ...
      if (billing.hasOwnProperty("pincode")) {
        ...
        // if pincode is invalid
        if (!isValidPincode(billing.pincode) || typeof (billing.pincode) !== "number") {
          return res.status(400).send({
            status: false,
